...
class ApiClient {
  ...
  Future<Map<String, dynamic>> _getDataOfResponse(
    ...
    if (response.statusCode == 401) {
      if (json['status_code'] == 7) {
        _errorMessage = 'Указан неверный API key для аунтентификации.';
        throw ApiClientException(ApiClientExceptionType.Other);
      } else if (json['status_code'] == 30) {
        _errorMessage =
  ...
  Future<String> _validateUser(
    ...
    try {
      assert(requestToken.length == 40 && requestToken.runtimeType == String);
  ...
  Future<String> _makeSession({required String requestToken}) async {
    ...
    final json = await _postMethod(url, parameters);
    if (json['success'] == false) _errorMessage = '${json['status_message']}';
